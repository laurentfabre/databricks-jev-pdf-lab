"""Index retained evidence; literal diagnostics never prove truth or safe pruning.

Pure code. No network, inference, PDF processing, input mutation or repair.
"""
...
from table_compaction import StrictTables, pack_tables
...
class InlineText(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        need(tag in StrictTables.INLINE | {'br'}, 'Unsupported cell markup')
        if tag == 'br': self.parts.append('\n')

    def handle_endtag(self, tag):
        need(tag in StrictTables.INLINE, 'Unsupported cell closing tag')
...
def fragments(element):
    ...
    need(isinstance(content, str), 'Missing parser content')
    if element.get('type') != 'table':
        need(not re.search(r'<[A-Za-z/!]', content), 'Markup outside a table')
        return [{'cell': None, 'text': content}]
    packet = pack_tables(content)
    parts = []
    for ti, table in enumerate(packet['tables']):
        for ri, row in enumerate(table):
            for ci, cell in enumerate(row):
                inner = cell if isinstance(cell, str) else cell.get('th', cell.get('html'))
                parser = InlineText()
                parser.feed(inner); parser.close()
                parts.append({'cell': [ti, ri, ci], 'text': ''.join(parser.parts)})
    return parts
...
def audit(raw, parsed, schema, contract):
    ...
    descriptions = [(s['index'], s['element']['description']) for s in sources
